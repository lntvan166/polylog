...
- It appears in the title bar of any `polylog:` editor, and on a file row's right-click in
  the Changes pane.
...
The commit header in the Changes pane is a block above the tree, not the tree's first row:
the subject, `sha · author · time` and the message body. It carries the Copy SHA and
Copy Message context menu. The tree holds the folders and files.
